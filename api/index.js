...
const uniqueName = (name="result.txt")=>{
  const i = name.lastIndexOf(".");
  const rand = Date.now().toString(36);
  if (i === -1) return `${name}-${rand}`;
  return `${name.slice(0,i)}-${rand}${name.slice(i)}`;
};
...
function sendSmart(API, chatId, title, content, filename="result.txt") {
  ...
  form.append("document", Buffer.from(content), uniqueName(filename));
...
WhatsApp: ${OWNER.whatsapp}`
  ...
  (async ()=>{
    ...
    // ✅ FITUR OBF (DIKEMBALIKAN)
    if (text.startsWith("/obf ")) {
      const [, type, ...r] = text.split(" ");
      const input = await resolveInput(r.join(" "));
      if (!input) return;
      if (!OBF[type]) return;
      return sendSmart(API, chatId, "Obfuscated Code", OBF[type](input));
    }
  })().catch(console.error);
}

/* ================= ENCODE ================= */
const ENC = { /* TETAP SAMA */ };
const DEC = { /* TETAP SAMA */ };
